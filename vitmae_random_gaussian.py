...
from dataloader import BATCH_SIZE, square_binary_dataloader_gaussian
import torch
...
def train_model(num_epochs=100):
    ...
    for epoch in range(num_epochs):
        for idx, data in enumerate(square_binary_dataloader_gaussian):
            # ===================forward=====================
            data = data.to(device)
            output = model(data)
            ...
            if idx % 2500 == 0:
                torch.save(model.state_dict(), f'/pscratch/sd/h/hasitha/xrd/vitmae_random_gaussian/vitmae_random_gaussian_epoch_{epoch}_batch_{idx}.pth')
            if idx % 5 == 0:
...
torch.save(model.state_dict(), '/global/homes/h/hasitha/latent_xrd/vitmae_random_gaussian.pth')
print('Model saved')